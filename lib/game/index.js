...
function Game(){
  ...
  this.takeTurn = function(direction){
    const previousGrid = _.clone(this.board.grid);
    // console.log(this.board.grid[0]);
    this.move(direction);
    // if(this.boardNoChange(previousGrid)){ this.addTile() };
    this.addTile()
...
Game.prototype.boardNoChange = function(grid){
  var changed      = true;
  var previousGrid = _.flatten(grid, true);
  var currentGrid  = _.flatten(this.board.grid, true);
  for(var i = 0; i < 16; i++){
    // console.log(previousGrid[i].num)
    // console.log(currentGrid[i].num)
    if(currentGrid[i].num != previousGrid[i].num ){ changed = false };
  };
  return changed;
};
